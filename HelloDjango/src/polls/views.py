...
from django.http import HttpResponseRedirect #특정 주소로 이도오
...
def vote(request, question_id):
    #if request.methhod=="GET": GET방식으로 요청했을 때만 처리
    ...
    #return HttpResponse( 'question_id : {}, vote : {}'.format(question_id, request.post.get('vote')))
    # return HttpResponse('question_id : {}'.format(question_id))
...
def registerQ(request):
    ...
    if request.session['LoginState']==True:
        if request.method == "GET": #해당 뷰를 GET방식으로 요청시
            obj = QuestionForm() #QuestionForm에 객체를 생성 - HTML에서 form 태그 컨텐츠 안에 넣으면 됨
            return render(request, 'polls/registerQ.html/', { 'form' : obj })
        else: #해당 뷰를 POST방식으로 요청시
            #request.POST.get("") - form내에 있는 특정 변수값을 가져올 때
            #request.POST - form 내에 있는 모든 값을 사용
            ...
            if obj.is_valid(): #값이 유효한가?
                ...
                #obj.sace(commit = False) : QuestionForm을 Question객체로 변환
                ...
                return render(request, 'polls/registerQ.html',
                              {'form' : obj, 'error_meesage' : "비정상적인 값입니다."} )
    else: #세션에 loginstate값이 없는 경우(비로그인 상태)
        return render(request, 'polls/error.html', {'error':'접근할 관한이 없습니다.'})
...
def registerC(request, question_id):
    ...
    if question.customuser.id == request.session['username']:
        if request.method == "GET":
            #get_object_or_404(모델 클래스명, pk=id 값을 가진 변수)
            #입력한 모델클래스에서 객체를 찾아서 없는 경우 404Error 띄어준다
            question = get_object_or_404(Question, pk=question_id)
            obj = ChoiceForm()
            ...
            if obj.is_valid():
                ...
                choice.question = question #question 객체를 넘겨줘야한다
                choice.save()
                return HttpResponseRedirect(reverse('polls:detail', args=(question_id, )))
            else:
                return render(request, 'polls/registerC.html',
...
def deleteC(request, choice_id):
    ...
    if obj.question.customuser.id == request.session['username']:
        #obj : choice 객체
        #obj.question : choice 객체와 연결된 question 객체
...
def search(request):
    ...
    #글쓴이로 검색
        ...
        #CustomUser의 id 값이 content와 동일한 객체를 추출
        user = CustomUser.objects.filter(id=content)
        #customuser 변수의 값이 user 객체와 동일한 Question 객체를 추출
    ...
    #투표수로 검색
        ...
        return render(request, 'polls/error.html', {'error' : "검색타입이 정상적이지 않습니다."})
